process pipeline_start {
  ...
  echo "nextflow run ${PSGA_ROOT_PATH}/psga --run ${run} --ncov_workflow ${ncov_workflow} --filetype ${filetype} --metadata ${metadata} --scheme_repo_url ${scheme_repo_url} --scheme_dir ${scheme_dir} --scheme ${scheme} --scheme_version ${scheme_version} -resume \\\$1" >> ${session_id_file}
  """
}
