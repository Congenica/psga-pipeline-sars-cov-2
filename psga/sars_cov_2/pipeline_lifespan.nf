process pipeline_start {
  ...
  echo "nextflow run ${PSGA_ROOT_PATH}/psga -c ${PSGA_ROOT_PATH}/psga/sars_cov_2.config --run ${run} --sequencing_technology ${sequencing_technology} --metadata ${metadata} --scheme_repo_url ${scheme_repo_url} --scheme_dir ${scheme_dir} --scheme ${scheme} --scheme_version ${scheme_version} -resume \\\$1" >> ${session_id_file}
  """
}
