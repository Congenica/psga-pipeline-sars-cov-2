...
#
# The script assumes that there will be directories under COVID_PIPELINE_INPUT_PATH env var that
...
then
  echo "COVID_PIPELINE_INPUT_PATH not set!"
  exit 1
fi

complete_files=($(find ${COVID_PIPELINE_INPUT_PATH} -name "transfer[-_]complete*"))
