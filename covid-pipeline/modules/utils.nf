...
process store_notification_with_values_list{
  publishDir "${COVID_PIPELINE_NOTIFICATIONS_PATH}/${params.run}", mode: 'copy', overwrite: true
...
process store_mismatching_files{
    publishDir "${COVID_PIPELINE_MISSING_METADATA_PATH}/${params.run}", mode: 'copy', overwrite: true
