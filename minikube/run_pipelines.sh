...
for name in $PIPELINES; do
  for config in $(ls test_configs/$name); do
    ...
    kubectl -n psga-minikube exec $pipeline_pod -- bash -c "cd ${PSGA_ROOT_PATH}/integration_tests && pytest test_validation.py --expected-results-csv ${PSGA_ROOT_PATH}/jenkins/files/${pathogen}/expected_results/${ANALYSIS_RUN}/results.csv --results-csv ${OUTPUT_PATH}/results.csv --output-path ${OUTPUT_PATH} --pathogen ${pathogen} --sequencing-technology ${SEQUENCING_TECHNOLOGY} && cd -"
  done
done
